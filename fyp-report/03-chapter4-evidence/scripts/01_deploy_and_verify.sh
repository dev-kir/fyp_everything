...
echo ""
echo "=== Health Checks ===" | tee -a ../raw_outputs/01_deployment_log.txt

echo "Checking load balancer health..."
curl -s http://192.168.2.50:8081/health | jq . | tee -a ../raw_outputs/01_deployment_log.txt

echo "Checking load balancer metrics..."
curl -s http://192.168.2.50:8081/metrics | jq .replica_stats | tee -a ../raw_outputs/01_deployment_log.txt

echo "Checking web-stress health..."
curl -s http://192.168.2.50:8080/health | jq . | tee -a ../raw_outputs/01_deployment_log.txt

echo "Checking monitoring-agent health..."
curl -s http://worker-1:8082/health | jq . | tee -a ../raw_outputs/01_deployment_log.txt

echo ""
echo "All services deployed and verified!"
echo "Check ../raw_outputs/01_deployment_log.txt for details"
